...
@Resource(name = RestConstants.VERSION_1 + "/flags", order = 2, supportedOpenmrsVersions = { "1.8.*", "1.9.*", "1.10.*", "1.11.*", "1.12.*" })
...
public class FlagsResource extends BaseDelegatingReadableResource<Flag> {
	...
	public Flag getByUniqueId(final String uniqueId) {
		...
		final Flag flag = Context.getService(FlagService.class).getAllFlags().get(0);
		return flag;//returns first flag.
	}

	private List<Flag> getFlag(final String uniqueId) {
		final Patient patient = Context.getPatientService().getPatientByUuid(uniqueId);
		final FlagService flagService = Context.getService(FlagService.class);
		flagService.generateFlagsForPatient(patient);
